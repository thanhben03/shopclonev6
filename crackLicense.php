...
function checkLicenseKey($licensekey){
    ...
    if($results['status'] == "Active"){   
        $results['msg'] = "Gi?y phép h?p l?";
        $results['status'] = true;
        return $results;
    }
    if($results['status'] == "Invalid"){   
        $results['msg'] = "Gi?y phép kích ho?t không h?p l?";
        $results['status'] = false;
        return $results;
    }
    if($results['status'] == "Expired"){   
        $results['msg'] = "Gi?y phép mã ngu?n dã h?t h?n, vui lòng gia h?n ngay";
        $results['status'] = false;
        return $results;
    }
    if($results['status'] == "Suspended"){   
        $results['msg'] = "Gi?y phép c?a b?n dã b? t?m ngung";
        $results['status'] = false;
        return $results;
    }
    $results['msg'] = "Không tìm th?y gi?y phép này trong h? th?ng";
    $results['status'] = false;
    return $results;
}
...
if ($version != file_get_contents('http://api.cmsnt.co/version.php?version=SHOPCLONE6')){
    ...
    die("Không có phiên b?n m?i nh?t");
}
?>