...
<div class="col-lg-12">
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <div class="header-title">
                <h4 class="card-title"><?=__('MUA TÀI KHOẢN');?></h4>
            </div>
        </div>
        <div class="card-body">
            <style>
            .textSoDu {
                background:
                    url("<?=base_url('assets/img');?>/bg13_zps2612a7c5.gif") repeat scroll 0 0%,
                    url("<?=base_url('assets/img');?>/bg4_zps012f7fcf.gif") repeat scroll 0 0%,
                    url("<?=base_url('assets/img');?>/bg5_zps9146c4c3.gif") repeat scroll 0 0;
                color: white;
                font-weight: bold;
            ...
            <div class="text-right mb-3">
                ...
                <button class="btn btn-outline-danger"><i class="fas fa-wallet mr-1"></i><?=__('Số Dư');?>:
                    <b class="textSoDu"><?=format_currency($getUser['money']);?></b></button>
                <?php } else {?>
                <a type="button" href="<?=BASE_URL('client/login');?>" class="btn btn-outline-primary"><i
                        class="fas fa-sign-in-alt mr-1"></i><?=__('Đăng Nhập');?></a>
                <a type="button" href="<?=BASE_URL('client/register');?>" class="btn btn-outline-info"><i
                        class="fas fa-user-plus mr-1"></i><?=__('Đăng Ký');?></a>
                <?php }?>
            </div>
                            ...
                            <span class="d-flex align-items-center">
                                <div class="item item-circle bg-black-5 text-primary-light" style="min-width: 60px;">
                                    ...
                                    <span class="text-primary text-center font-weight-bolder"
                                        style="font-size: 14px;"><?=format_cash($CMSNT->get_row("SELECT COUNT(id) FROM `accounts` WHERE `product_id` = '".$product['id']."' AND `buyer` IS NULL AND `status` = 'LIVE' ")['COUNT(id)']);?></span>
                                    <?php endif?>
                                </div>
